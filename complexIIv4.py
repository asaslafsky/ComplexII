...
# Definitions
RUN_TYPE = '100 ng/ml TNF' #Gives Titles to Saved Graphs: name according to what is changed i.e. protein fluctuations
...
for obs in model.observables:
    plt.figure()
    plt.plot(tspan / 60, ode_sim_res.observables[obs.name])
    plt.xlabel("Time (in hr)", fontsize=15)
    plt.ylabel("%s [Molecules/Cell]" % obs.name, fontsize=15)
...
all_times = [420, 600, 1440, 2160] #Array of time points of interest: 24, 48, 72
